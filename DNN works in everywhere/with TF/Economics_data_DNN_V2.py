...
''' the logic to define which parameter matter the most is not correct.

    weight_each = []
    weight_each_total_list = []
    weight_each_total = 0
    for i in range(len(b)):
        listi = []
        for j in range(len(c)):
            weight_each_total += b[i][j]*c[j][0]
            if b[i][j]*c[j][0] > 0.05 or b[i][j]*c[j][0] < -0.05:
                listi.append(round(b[i][j]*c[j][0],3))
            else:
                 listi.append(0)
        weight_each_total_list.append(weight_each_total)
        weight_each.append(listi)


#写Excel by DataFrame
dataFrame = pd.DataFrame(weight_each)
dataFrame.insert(0,'weight_each_total_list',weight_each_total_list)
dataFrame.index = inputs_name
dataFrame = dataFrame.sort_values('weight_each_total_list', ascending= False)
dataFrame.to_excel("test1.xlsx",sheet_name='weights_recording')
print(dataFrame)
'''


''' write into excel with xlwt  !!not good!! -----------------------------------------
import xlwt

def write_excel():
    f = xlwt.Workbook()
    sheet1 = f.add_sheet('Data parameters',cell_overwrite_ok=True)
    row0 = [i for i in range(1,61)]
    colum0 = inputs_name
    #写第一行前两个
    sheet1.write(0,0,"Parameter",set_style('Times New Roman',220,True))
    sheet1.write(0,1,"Total Weight",set_style('Times New Roman',220,True))
    
    #写第一行
    for i in range(0,len(row0)):
        sheet1.write(0,i+2,row0[i],set_style('Times New Roman',220,True))
    #写第一列
    for i in range(0,len(colum0)):
        sheet1.write(i+1,0,colum0[i],set_style('Times New Roman',220,True))

    for i in range(len(weight_each)):
        for j in range(len(weight_each[i])):
            sheet1.write(i+1,1,str(weight_each_total_list[i]))
            sheet1.write(i+1,j+2,str(weight_each[i][j]))
    print(pd.DataFrame(sheet1))
    f.save('test.xls')
'''
